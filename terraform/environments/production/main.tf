module "vpc" {
  ...
  single_nat_gateway   = false # production: one NAT per AZ so an AZ outage doesn't kill egress
}
...
module "ecs_frontend" {
  ...
  min_capacity     = 2
  max_capacity     = 6
  use_fargate_spot = false # production stays on regular Fargate - no surprise reclaims
