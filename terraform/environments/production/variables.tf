...
variable "frontend_image" {
  type        = string
  description = "Frontend image URI incl. tag. Set by CI at deploy time via -var (only after the approval gate)."
}

variable "backend_image" {
  type        = string
  description = "Backend image URI incl. tag. Set by CI at deploy time via -var (only after the approval gate)."
}
